...
const Home: NextPage<Props> = ({ currency }) => {
  ...
  return (
    <main
      className="flex min-h-screen w-full flex-1 flex-col items-center justify-center bg-black px-12 font-mono text-white"
      onClick={handleClick}
    >
      ...
      <div className="mt-4 flex flex-col items-center justify-center">
        <article className="flex flex-row items-center justify-between gap-2 rounded bg-green-500 px-2 py-1 hover:cursor-pointer md:gap-4">
          <p className="font-bold uppercase">Compra</p>
          <Image src="/arrow-right.svg" width={24} height={24} layout="fixed" />
          <p className="font-bold">${currency.compra}</p>
        </article>
        <article className="mt-2 flex flex-row items-center justify-between gap-2 rounded bg-red-500 px-2 py-1 hover:cursor-pointer md:gap-4">
